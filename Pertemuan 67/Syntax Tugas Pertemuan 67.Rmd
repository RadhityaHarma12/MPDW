...
```{r}
databaru.train <- databaru.ts[1:90]
databaru.test <- databaru.ts[91:120]
...
Plot Boxcox menunjukkan nilai \*rounded value\* (\$\\lambda\$) optimum sebesar \*\*-0.54\*\* dan pada selang kepercayaan 95% nilai memiliki batas bawah \*\*-1.00\*\* dan batas atas \*\*-0.09\*\*. Selang tersebut tidak memuat nilai satu sehingga dapat dikatakan bahwa data bangkitan tidak stasioner dalam ragam.
...
eacf(databaru.diff)
```
Identifikasi model menggunakan plot EACF dilakukan dengan melihat ujung segitiga pada pola segitiga nol. Dalam hal ini model tentatif yang terbentuk adalah ARIMA(3,1,2), ARIMA(3,1,3), ARIMA(0,1,4), dan ARIMA(1,1,4).
...
### ARIMA(3,1,2)
```{r}
model3.da=Arima(databaru.diff, order=c(3,1,2),method="ML")
summary(model3.da) 
...
par(mfrow = c(1,1))
```
Berdasarkan plot kuantil-kuantil normal, secara eksplorasi ditunjukkan sisaan tidak menyebar normal ditandai dengan titik titik yang cenderung tidak mengikuti garis $45^{\circ}$. Kemudian dapat dilihat juga lebar pita sisaan yang cenderung tidak sama menandakan bahwa sisaan memiliki ragam yang heterogen. Plot ACF dan PACF sisaan ARIMA(0,1,1) signifikan pada lag ke-1 yang menandakan tidak saling bebas. Kondisi ini akan diuji lebih lanjut dengan uji formal.
...
Berdasarkan uji Ljung-Box tersebut, didapat *p-value* sebesar 0.002809 yang lebih kecil dari taraf nyata 5% sehingga tolak $H_0$ dan menandakan bahwa sisaan tidak saling bebas.
### 3. Sisaan homogen
```{r}
...
Berdasarkan uji Ljung-Box terhadap sisaan kuadrat tersebut, didapat *p-value* sebesar 0.07105 yang lebih dari taraf nyata 5% sehingga tolak $H_0$ dan menandakan bahwa ragam sisaan homogen.
...
Berdasarkan uji-ttersebut, didapat *p-value* sebesar 0.7896 yang lebih besar dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa nilai tengah sisaan sama dengan nol.
...
model1b.ma2=Arima(databaru.ts, order=c(0,1,2),method="ML")
summary(model1b.ma2) #327.24
lmtest::coeftest(model1b.ma2) 

#model yang dipilih adalah model awal, yaitu ARIMA(1,1,1)
```
Berdasarkan kedua model hasil *overfitting* di atas, model ARIMA(1,1,1) dan ARIMA(0,1,2) memiliki AIC yang lebih besar dibandingkan dengan model ARIMA(0,1,1) dan parameter model ARIMA(0,1,2) tidak seluruhnya signifikan. Oleh karena itu, model ARIMA(0,1,1) akan tetap digunakan untuk melakukan peramalan.
...
```{r}
pt_1 <- databaru.train[90] #nilai akhir data
...
Pada Accuracy data latih mendapatkan mape sebesar 93.44561 dan pada data uji sebesar 45.36769 Sehingga mape pada data latih lebih besar dibandingkan data uji.